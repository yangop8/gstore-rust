...
mod tests {
    ...
    #[test]
    fn parses_tab_separated_iris() {
        let t = parse_line("<root>\t<name>\t<node0>\t.", 1)
            .unwrap()
            .unwrap();
        assert_eq!(t.subject, Term::iri("root"));
        assert_eq!(t.predicate, Term::iri("name"));
        assert_eq!(t.object, Term::iri("node0"));
    }

    #[test]
    fn parses_literal_with_spaces() {
        let t = parse_line("<root>\t<name>\t\"Bookug Lobert\"\t.", 1)
            .unwrap()
            .unwrap();
        assert_eq!(t.object, Term::plain_literal("Bookug Lobert"));
    }

    #[test]
    fn parses_typed_literal() {
        let line =
            "<http://example/alice> <http://x/salary> \"2500\"^^<http://www.w3.org/2001/XMLSchema#integer> .";
    ...
    fn parses_lang_literal() {
        let t = parse_line("<a> <b> \"chat\"@fr .", 1).unwrap().unwrap();
        assert_eq!(
            t.object,
            Term::Literal {
                value: "chat".into(),
    ...
    fn parses_blank_nodes() {
        let t = parse_line("_:b0 <p> _:b1 .", 1).unwrap().unwrap();
        assert_eq!(t.subject, Term::blank("b0"));
        assert_eq!(t.object, Term::blank("b1"));
    }

    #[test]
    fn handles_escapes_in_literal() {
        let t = parse_line(r#"<a> <b> "line1\nline2 \"q\"" ."#, 1)
            .unwrap()
            .unwrap();
        assert_eq!(t.object, Term::plain_literal("line1\nline2 \"q\""));
    }

    #[test]
    fn unicode_escape_in_literal() {
        let t = parse_line(r#"<a> <b> "é" ."#, 1).unwrap().unwrap();
        assert_eq!(t.object, Term::plain_literal("é"));
    }

    #[test]
    fn missing_trailing_dot_is_tolerated() {
        let t = parse_line("<a> <b> <c>", 1).unwrap().unwrap();
        assert_eq!(t.object, Term::iri("c"));
    }
    ...
    fn trailing_comment_after_dot_is_ok() {
        let t = parse_line("<a> <b> <c> . # note", 1).unwrap().unwrap();
        assert_eq!(t.subject, Term::iri("a"));
    }

    #[test]
    fn unterminated_iri_errors_with_line() {
        let e = parse_line("<a> <b> <c", 7).unwrap_err();
        match e {
            GStoreError::RdfParse { line, .. } => assert_eq!(line, 7),
            other => panic!("expected RdfParse, got {other:?}"),
        }
    }

    #[test]
    fn parse_str_collects_multiple_lines() {
        let doc = "<a> <p> <b> .\n# comment\n<a> <p> \"v\" .\n";
